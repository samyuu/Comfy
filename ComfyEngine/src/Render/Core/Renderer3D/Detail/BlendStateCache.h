	...
	public:
		BlendStateCache()
		{
			for (size_t src = 0; src < BlendFactorCount; src++)
			{
				...
				{
					states[src][dst] = std::make_unique<D3D11BlendState>(
						GlobalD3D11,
						D3DBlendFactors[src], 
						D3DBlendFactors[dst], 
						D3D11_BLEND_INV_DEST_ALPHA, 
					...
					D3D11_SetObjectDebugName(states[src][dst]->BlendState.Get(), 
						"Renderer3D::BlendState::%s-%s", Graphics::BlendFactorNames[src], Graphics::BlendFactorNames[dst]);
				}
			}
		}

		D3D11BlendState& GetState(Graphics::BlendFactor source, Graphics::BlendFactor destination)
		{
			return *states[static_cast<size_t>(source)][static_cast<size_t>(destination)];
		}

	private:
		std::array<std::array<std::unique_ptr<D3D11BlendState>, BlendFactorCount>, BlendFactorCount> states;
	};
}
