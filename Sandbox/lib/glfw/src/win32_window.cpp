	...
	if (!window)
	{
		...
		case WM_DEVICECHANGE:
		{
			if (_glfw.callbacks.joystick)
				_glfw.callbacks.joystick(0, wParam == DBT_DEVICEARRIVAL ? GLFW_CONNECTED : DBT_DEVICEREMOVECOMPLETE ? GLFW_DISCONNECTED : 0);
			break;
		}
		}
...
{
	const HMONITOR handle = MonitorFromWindow(window->win32.handle, MONITOR_DEFAULTTONEAREST);
...
int _glfwPlatformCreateStandardCursor(_GLFWcursor* cursor, int shape)
{
	...
	switch (shape)
	{
	case GLFW_NORMAL_CURSOR:	id = OCR_NORMAL;	break;
	case GLFW_IBEAM_CURSOR:		id = OCR_IBEAM;		break;
	case GLFW_WAIT_CURSOR:		id = OCR_WAIT;		break;
	case GLFW_CROSS_CURSOR:		id = OCR_CROSS;		break;
	case GLFW_UP_CURSOR:		id = OCR_UP;		break;
	case GLFW_SIZENWSE_CURSOR:	id = OCR_SIZENWSE;	break;
	case GLFW_SIZENESW_CURSOR:	id = OCR_SIZENESW;	break;
	case GLFW_SIZEWE_CURSOR:	id = OCR_SIZEWE;	break;
	case GLFW_SIZENS_CURSOR:	id = OCR_SIZENS;	break;
	case GLFW_SIZEALL_CURSOR:	id = OCR_SIZEALL;	break;
	case GLFW_NO_CURSOR:		id = OCR_NO;		break;
	case GLFW_HAND_CURSOR:		id = OCR_HAND;		break;
